...
func (c *Controller)initialize(something *samplecrdcontrollerv1alpha1.Something) (trigger bool, err error) {
	trigger = false
	err = nil
...
func (c *Controller) finalize(something *samplecrdcontrollerv1alpha1.Something) (trigger bool, err error) {
	trigger = false
	err = nil
	if(something.DeletionTimestamp != nil) {
		...
		if got {
			...
			_, upErr := c.clientset.SamplecrdcontrollerV1alpha1().Somethings(something.Namespace).Update(somethingCopy)
			if upErr != nil {
				err = upErr
